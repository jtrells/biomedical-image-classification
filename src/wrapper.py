...
def extract_and_update_features(model_path, parquet_path, base_img_dir, label_col='label', batch_size=32, num_workers=16):
    ...
    update_features(model, parquet_path, base_img_dir, label_col=label_col,
                    seed=42, batch_size=batch_size, num_workers=num_workers)
    # try:
...
def get_data(db, vil_path, taxonomy, classifier, reducer_name, version='latest', subset='all', num_dimensions=2, add_hits=True, label_col='label'):
    ...
    if (version == 'latest'):
        cursor = db.classifiers.find(
            {'taxonomy': taxonomy, 'classifier': classifier}).sort([('version', DESCENDING)])
        try:
            classifier_info = cursor.next()
        except:
            raise Exception('no classifier available for parameters')
    else:
        classifier_info = db.classifiers.find_one(
            {'taxonomy': taxonomy, 'classifier': classifier, 'version': version})

    parquet_path = Path(vil_path) / 'files' / taxonomy / \
        classifier_info['dataset']
...
def get_figure_neighbors(db, vil_path, taxonomy, classifier, version, img_path, n_neighbors, subset='all'):
    ...
    df = pd.read_parquet(parquet_path)
    # filter the df by split if required
    subset_col = 'split_set'
    subset = None if subset == 'all' else subset
    if subset != None:
        df = df[df[subset_col] == subset].reset_index(drop=True)
    ...
    neighbors_df = get_neighbors_by_index(df, index, n_neighbors=n_neighbors)
    # now merge with all_df to get the full_label if it exists
    all_path = Path(vil_path) / 'files' / taxonomy / 'all.parquet'
    all_df = pd.read_parquet(all_path)
    merged_df = neighbors_df.merge(all_df, on="img", how="inner")
    merged_df = merged_df.drop(
        ['caption_y', 'width_y', 'height_y', 'img_path_y'], axis=1)
    merged_df = merged_df.rename(columns={"label_x": "label",
                                          "img_path_x": "img_path",
...
def upsert_label_updates(db, images):
    ...
    for image in images:
        ...
        print("upsert id", result.upserted_id)
        print("")
