...
def extract_and_update_features(model_path, csv_path, base_img_dir, batch_size=32, num_workers=16):
    model_path = Path(model_path)

    model = ResNetClass.load_from_checkpoint(model_path)
    df = pd.read_csv(csv_path, sep='\t')
    le_encoder, _ = label_encoder_target(df,target_col='higher_modality')
