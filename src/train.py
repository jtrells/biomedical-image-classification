...
def main():
    parser = argparse.ArgumentParser(description='High Level Biomedical Image Classification')
    ...
    parser.add_argument('--project_name', type=str, default='high-level biomedical modality', 
    ...
    classes.sort()
    provider = TrainDataLoader(args.dataset_path, args.labels_path, classes, seed=args.seed)
    ...
    if args.architecture == 'resnet50':
        ...
        model = EfficientNet.from_pretrained('efficientnet-b0', num_classes=args.num_output_classes)
        for param in model.parameters():
             param.requires_grad = False
        model._fc.weight.requires_grad = True
        model._fc.bias.requires_grad = True
    
    run = ExperimentalRun(model, provider, args, notes=args.notes, tags=['clef', 'modality'])
    run.train()
